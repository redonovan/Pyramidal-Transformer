...
class PyramidalTransformerLayer(keras.layers.Layer):
    '''returns outputs with half as many timesteps as the inputs'''
    def __init__(self, nheads, layer_dim, ffn_dim, dropout_rate, **kwargs):
        super(PyramidalTransformerLayer, self).__init__(**kwargs)
        ...
        self.rl1 = layers.Reshape((-1, layer_dim*2))
        self.rl2 = layers.Reshape((-1, 2))
        self.dl1 = layers.Dense(ffn_dim, activation='relu')
...
class TestModel(keras.Model):
    ...
    def call(self, inputs, training):
        ...
        c = self.condenser(h, mask=hmask) # -> (batch, lis_dim)
