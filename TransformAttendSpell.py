...
num_epochs   = 1     # free choice, number of epochs of training
...
class Transformer(keras.layers.Layer):
    def __init__(self, nlayers, nheads, tran_dim, dropout_rate, max_frames, mel_dim, **kwargs):
        ...
        self.lays  = [TransformerLayer(nheads, tran_dim, dropout_rate) for _ in range(nlayers)]
    #
    def call(self, inputs, mask, training):
        ...
        # return shape (batch, frames, tran_dim)
        return x
    #
...
class Listener(keras.layers.Layer):
    def __init__(self, lis_layers, lis_dim, nheads, dropout_rate, max_frames, mel_dim, **kwargs):
        super(Listener, self).__init__(**kwargs)
        # for backwards compatibility the transformer dim is lis_dim*2
        self.tran_dim = lis_dim*2
        # the Transformer encoder
        self.tr = Transformer(lis_layers, nheads, self.tran_dim, dropout_rate, max_frames, mel_dim)
        #
    def call(self, inputs, mask, training):
        ...
        h = self.tr(inputs, mask, training)
...
class TestModel(keras.Model):
    ...
    def __init__(self, lis_layers, lis_dim, nheads, dropout_rate, max_frames, mel_dim, num_speakers,
                 **kwargs):
        super(TestModel, self).__init__(**kwargs)
        self.listener = Listener(lis_layers, lis_dim, nheads, dropout_rate, max_frames, mel_dim)
...
# tmodel = TestModel(lis_layers, lis_dim, nheads, dropout_rate, max_frames, mel_dim, 20000)
# tmodel.compile(
...
#
# This works, but batch_size has to be 2 with Transformers.
...
class LASModel(keras.Model):
    def __init__(self, lis_dim, lis_layers, dec_dim, att_dim, voc_dim, frac_pyp, max_dec, max_frames,
                 mel_dim, mloss_wt, nheads, dropout_rate, **kwargs):
